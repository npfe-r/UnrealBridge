...
- **`rename` writes the redirect to disk synchronously, but later mutations on the same source ini can silently drop it.** Verified with file-level snapshots on UE 5.7: a follow-up `add_gameplay_tag` (or another mutation that touches `UGameplayTagsSettings`) sometimes triggers a re-serialization where redirects whose `OldTagName` no longer matches an in-memory tag node get pruned. The in-memory manager retains the redirect for the rest of the session, so lookups succeed during the same editor run — but the **next editor restart** will fail to load the missing redirect, and any stale references to the old name will silently un-resolve. **Mitigation:** after a `rename_gameplay_tag` call, immediately read the source ini and confirm the `+GameplayTagRedirects=(OldTagName="X",NewTagName="Y")` line is still present; if it's gone, append it back manually before performing further mutations or restarting the editor.
